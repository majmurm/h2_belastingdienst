...
def compute_group_mean(model, size_cat, age_cat, has_advisor=None):
    ...
    group_agents = [
        ...
        if a.size_cat == size_cat and a.age_cat == age_cat and (has_advisor is None or a.has_advisor == has_advisor)
    ]
...
class SMEAgent(Agent):
    ...
    """
    def __init__(self, model, size_cat: str, age_cat: str, propensity: float, turnover: float, tax_rate: float, has_advisor: bool):
...
class SMEComplianceModel(Model):
    ...
    def __init__(
        ...
        for i in range(N):
            ...
            if has_advisor: mu = min(1.0, mu + 0.02)
            else: mu = max(0.0, mu - 0.02)
            ...
            SMEAgent(self, size_cat=s, age_cat=a, propensity=propensity, turnover=turnover, tax_rate=tax_rate, has_advisor=has_advisor)
        ...
        for s in self.size_order:
            for a in self.age_order:
                for adv in advisor_states:
                    ...
                    model_reporters[label] = lambda m, s=s, a=a, adv=adv: compute_group_mean(m, s, a, adv)
...
if __name__ == "__main__":
    ...
    print(f"ROI Ratio (Return per €1 spent on interventions): € {roi_ratio:.2f}") # ROI per USD spent by IRS: 2 - 12USD
    print("="*40)
    ...
    for col in sorted(df.columns):
        if " - " in col:  # This selects "Micro - Young", "Small - Old", etc.
            ...
            ax1.plot(df.index, df[col], label=col, linewidth=width, linestyle=linestyle, alpha=alpha)
    
    # Add the Global Mean as a thicker, distinct line
    ax1.plot(df.index, df["Mean Propensity"], color='black', linewidth=2.5, label="Global Mean")
